...
![](img/place/density_scale_models.png){#fig-density-zip}

<!-- TODO: pull out concrete values from figures and coefficient tables. -->

Across all four outcomes, a higher zip-code-level prevalence of same-sex couples is consistently associated with a greater sense of community and belonging for LGBQ people.

As @fig-ssc-scales-zip show

(all models discussed here use the best-fitting specifications for density)

quarter-point increase in community connectedness (coef:)

and half-point increase in social wellbeing (coef:)

(the two scales have different ranges, so the units are not comparable).

These strong associations are replicated in the individual question responses.

@fig-ssc-q53-zip

move from 1% to 10%, the predicted probability of "agree strongly" more than doubles from around 15% to 40%, and probability disagreeing at all falls from 30% to near 10%.

@fig-ssc-q04-zip

move from 1% to 10%, the predicted probability of "strongly disagree" also nearly doubles, to almost 40%.



![](img/place/pct_ssc_scales.png){#fig-ssc-scales-zip}

![](img/place/pct_ssc_q53.png){#fig-ssc-q53-zip}

![](img/place/pct_ssc_q04.png){#fig-ssc-q04-zip}

by contrast, not much at all at the MSA level. (put these figures in the appendix?)

%-ssc only social wellbeing scale

(Notably, even a simple varying-intercepts model with random effects - a completely different approach to capturing differences between MSAs - doesn't turn up much.)

finally, not the focus, but worth noting:
